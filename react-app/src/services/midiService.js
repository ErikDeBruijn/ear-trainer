// services/midiService.js
import { WebMidi } from 'webmidi';
import { lumiService } from './lumiService.js';
...
class MidiService {
    constructor() {
        ...
        this.activeInputs = [];
        this.activeOutputs = [];
        this.out = null; // Legacy single output for compatibility
        ...
        this.activeInputs.forEach(device => {
            ...
            device.addListener("noteon", "all", e => {
                this.noteCallbacks.forEach(cb => 
                    cb({ type: "on", note: e.note.number, name: e.note.identifier, velocity: e.velocity })
                );
            });
            device.addListener("noteoff", "all", e => {
                this.noteCallbacks.forEach(cb => 
                    cb({ type: "off", note: e.note.number, name: e.note.identifier, velocity: e.velocity })
                );
            });
        });
    }
